...
select 
	...
	substr( m.M_JUMIN , 1 , 6 ) || '-' || substr( m.M_JUMIN , 7 , 7 ) as jumin ,
	m.M_CITY ,
	p.P_TEL1 || '-' || p.P_TEL2 || '-' || p.P_TEL3
	from TBL_MEMBER_202005 m , TBL_PARTY_202005 p 
	where m.P_CODE = p.P_CODE;

select m.M_NO , m.M_NAME , p.P_NAME , case when m.P_SCHOOL = '1' then '고졸' when m.P_SCHOOL = '2' then '학사' when m.P_SCHOOL = '3' then '석사' when m.P_SCHOOL = '4' then '박사'end as school , substr( m.M_JUMIN , 1 , 6 ) || '-' || substr( m.M_JUMIN , 7 , 7 ) as jumin , m.M_CITY ,	p.P_TEL1 || '-' || p.P_TEL2 || '-' || p.P_TEL3 from TBL_MEMBER_202005 m , TBL_PARTY_202005 p where m.P_CODE = p.P_CODE;
